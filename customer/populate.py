...
def populate():
    ...
    customer2, created = Customer.objects.get_or_create(user=user2, operator=zebras)
    ...
    transaction6, created = Transaction.objects.get_or_create(payment_type="Card", payment_method="VisaDebit", price=price6)
    discount6, created = Discount.objects.get_or_create(discount_type="%", discount_value="50", discount_description="Over 60 yrs")
    concession6, created = Concession.objects.get_or_create(id=record6, mode=train, operator=zebras, name="Senior Traveller", price=price6, discount=discount6, transaction=transaction6, valid_from_date_time=django.utils.timezone.now(), valid_to_date_time=django.utils.timezone.now()+datetime.timedelta(days=364), conditions="Over 60", customer=customer2)
