	...
	try:
		...
		for linked_account in linked_accounts:
			...
			for ticket in out_list:
				...
				global date_format
				if operator.name == "PSD Buses":
					date_format = '%Y-%m-%dT%H:%M:%SZ'
					mode.short_desc = mode.short_desc.capitalize()
...
def getTicketPurchase(ticket, mode, operator, recordid, cust, loc_from, loc_to):
	...
	booking_date_time = formatdt(ticket['booking_date_time'], date_format)
	travel_from_date_time = formatdt(ticket['travel_from_date_time'], date_format)
	travel_to_date_time = formatdt(ticket['travel_to_date_time'], date_format)
...
def getTicketConcession(ticket, mode, operator, recordid, cust):
	...
	valid_from_date_time = formatdt(ticket['valid_from_date_time'], date_format)
	valid_to_date_time = formatdt(ticket['valid_to_date_time'], date_format)
...
def getTicketUsage(ticket, mode, operator, recordid, cust, loc_from, loc_to):
	...
	date_time = formatdt(ticket['travel_from']['date_time'], date_format)
	reference = ticket['travel_from']['reference']
	uft1 = UsageFromTo(location=loc_from, date_time=date_time, reference=reference)
	date_time = formatdt(ticket['travel_to']['date_time'], date_format)
	reference = ticket['travel_from']['reference']
...
def getTransaction(ticket, price):
	global date_format
	date_time = formatdt(ticket['date_time'], date_format)
